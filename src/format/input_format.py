...
from src.utils.utils import add_period_for_lmbff
...
class BaseInputFormat(ABC):
    ...
    def parse_answer(self, response: str):
        parsed_answer = self._parse_answer(response=response)
...
#     def _format_input(self, example: InputExample ) -> str:
#         return "Input: " + example.input.strip() + "\nOutput: "
...
class ICLSentimentNaturalInputFormat(BaseInputFormat):
    ...
    def _parse_answer(self, response: str) -> str:
        ...
        try:
            response_list = [response for response in response_list if len(response.strip()) > 0]
            parse_response = response_list[0].replace("The sentiment is", "")
            return parse_response.strip()
        except:
            return response
...
class ICLSentimentNaturalInputFormat2(BaseInputFormat):
    ...
    def _format_input(self, example: InputExample ) -> str:
        sentence = add_period_for_lmbff(example.input.rstrip())
...
class ICLHateSpeechInputFormat(BaseInputFormat):
    ...
    def _parse_answer(self, response: str) -> str:
        ...
        try:
            response_list = [response for response in response_list if len(response.strip()) > 0]
            parse_response = response_list[0].replace("Is this hate speech?", "")
            return parse_response.strip()
        except:
            return response
...
class TopicHateSpeechInputFormat(BaseInputFormat):
    ...
    def _parse_answer(self, response: str) -> str:
        ...
        try:
            response_list = [response for response in response_list if len(response.strip()) > 0]
            parse_response = response_list[0].replace(f"Is this hate speech about {self.topic}?", "")
            return parse_response.strip()
        except:
            return response
...
class ICLTweetStanceInputFormat(BaseInputFormat):    
    ...
    def _parse_answer(self, response: str) -> str:
        ...
        try:
            response_list = [response for response in response_list if len(response.strip()) > 0]
            parse_response = response_list[0].replace("The answer is:", "")
            return parse_response.strip()
        except:
            return response
...
class TRECNaturalInputFormat(BaseInputFormat):
    ...
    def _parse_answer(self, response: str) -> str:
        ...
        try:
            response_list = [response for response in response_list if len(response.strip()) > 0]
            parse_response = response_list[0].replace("The topic is", "")
            return parse_response.strip()
        except:
            return response
...
class NLINaturalInputFormat(BaseInputFormat):
    name = 'nli1'
    # TODO: add convert_to_true_false: 增加不同映射
    convert_to_true_false = True
